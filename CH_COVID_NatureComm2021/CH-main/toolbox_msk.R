...
get_formula <- function(logit, clean = TRUE) {
...
plot_table_forest <- function(class_results, omit_columns = c(), title= "", digits = 2, 
  ...
  output <- kable(class_results, caption = title, digits = digits) %>%
    ...
    column_spec(c(1:length(index_col)), bold = TRUE)
  
  return(output)
}

summar = function(model, truncate_p = TRUE, p_het = NULL, CI = TRUE, signif = FALSE, n = FALSE, latex = TRUE, ref = TRUE, sep = FALSE, format = TRUE) {
...
do_glm_and_plot_or <- function(data = NULL, response = NULL, terms_for_formula = NULL,  terms_to_iterate = NULL, terms_to_plot = NULL, max_OR = NULL, plot_name = NULL, width = 10, height = 4, term_factor = FALSE, factors_for_term = NULL){
  ...
  if(term_factor){
    for (trm in terms_to_iterate) {
      ...
      for(i in factors_for_term){
        fctr_term <- paste(trm, i, sep = "")
        logit_gene_var = rbind(logit_gene_var, 
                               cbind(
                                 tidy(logit, exponentiate = TRUE, conf.int = TRUE, conf.level = .95) %>%
                                   filter(term == fctr_term) %>% 
                                   mutate(term_analyzed = fctr_term),
